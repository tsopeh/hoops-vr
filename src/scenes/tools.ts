...
function requireScriptForNodes (scene: Scene, scriptsMap: ISceneScriptMap, nodes: (Node | Scene)[]): void {
  ...
  for (const i of initializedNodes) {
    ...
    for (const link of soundLinks) {
      switch (link.type) {
        case 'global':
          n[link.propertyKey] = scene.mainSoundTrack.soundCollection.find((s) => s.name === link.soundName && !s.spatialSound)
          break
        case 'spatial':
          n[link.propertyKey] = scene.mainSoundTrack.soundCollection.find((s) => s.name === link.soundName && s.spatialSound)
          break
        default:
          n[link.propertyKey] = scene.getSoundByName(link.soundName)
          break
      }
    }
